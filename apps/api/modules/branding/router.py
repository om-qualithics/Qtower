...
from apps.api.modules.branding.schemas import (
    BrandingConfigOut,
    BrandingConfigUpdate,
    BrandingPublicConfigOut,
    LicenseStatusOut,
)
...
def _to_public_config_out(config) -> BrandingPublicConfigOut:
    if config is None:
        return BrandingPublicConfigOut(
...
def _require_branding_manager(misty_session: str | None):
...
@router.get("/config", response_model=BrandingPublicConfigOut)
def get_config():
    """Public - the login page needs branding before anyone is signed in.
    Deliberately returns only the narrow public subset (name/logo/colors/
    enabled_modules) - admin-authored fields (email_templates,
    tool_assessment_prompt, escalation_notify_override_email) are internal
    configuration and must never be readable by an anonymous caller. See
    GET /branding/admin-config for those, gated behind branding.manage."""
    org = identity_service.get_org()
    return _to_public_config_out(branding_service.get_config(org))


@router.get("/admin-config", response_model=BrandingConfigOut)
def get_admin_config(misty_session: str | None = Cookie(default=None)):
    """Full config including admin-authored fields - same authenticated
    audience as PATCH below. Used by Settings sections (email templates,
    AI precheck prompt) that need to read the current value, not just
    write a new one."""
    _require_branding_manager(misty_session)
    org = identity_service.get_org()
